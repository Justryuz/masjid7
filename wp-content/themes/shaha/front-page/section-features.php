<div class="front_page_section front_page_section_features<?php
			$shaha_scheme = shaha_get_theme_option('front_page_features_scheme');
			if (!shaha_is_inherit($shaha_scheme)) echo ' scheme_'.esc_attr($shaha_scheme);
			echo ' front_page_section_paddings_'.esc_attr(shaha_get_theme_option('front_page_features_paddings'));
		?>"<?php
		$shaha_css = '';
		$shaha_bg_image = shaha_get_theme_option('front_page_features_bg_image');
...
?>><?php
	// Add anchor
	$shaha_anchor_icon = shaha_get_theme_option('front_page_features_anchor_icon');	
	$shaha_anchor_text = shaha_get_theme_option('front_page_features_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_features_inner<?php
			if (shaha_get_theme_option('front_page_features_fullheight'))
				echo ' shaha-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$shaha_css = '';
			$shaha_bg_mask = shaha_get_theme_option('front_page_features_bg_mask');
			$shaha_bg_color = shaha_get_theme_option('front_page_features_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$shaha_caption = shaha_get_theme_option('front_page_features_caption');
			if (!empty($shaha_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo !empty($shaha_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($shaha_caption, 'shaha_kses_content' ); ?></h2><?php
			}
		
			// Description (text)
			$shaha_description = shaha_get_theme_option('front_page_features_description');
			if (!empty($shaha_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo !empty($shaha_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($shaha_description), 'shaha_kses_content' ); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_features_output"><?php 
				if (is_active_sidebar('front_page_features_widgets')) {
					...
					if (!shaha_exists_trx_addons())
						...
						shaha_customizer_need_widgets_message('front_page_features_caption', 'ThemeREX Addons - Services');
				}
			?></div>
		</div>
	</div>
